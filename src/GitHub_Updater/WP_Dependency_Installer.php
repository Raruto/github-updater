 ...
 */
	class WP_Dependency_Installer {
		 ...
		 */
		public function run( $plugin_path = null ) {
			if ( $plugin_path != null && file_exists( $plugin_path . '/wp-dependencies.json' ) ) {
				$config = file_get_contents( $plugin_path . '/wp-dependencies.json' );
				if ( empty( $config ) || null === ( $config = json_decode( $config, true ) ) ) {
					return;
				}
...
// WP_Dependency_Installer::instance()->register("put json file here" );
